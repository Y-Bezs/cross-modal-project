def Cat_all(sub):
        ...
        data_name = 'full'
        path_data = os.path.join(result_path,data_name +'_ica-1.fif') 
...
# exclude trials before failed questions
        ...
        path_outfile = os.path.join(result_path,data_name+'_epo-right.fif') 
        ...
        filename_fig = op.join(result_path, 'TOPO_mag_pic.png')
        ...
        ERP_all=np.mean(evoked_pic.get_data(),axis=0)
        fig4=plt.figure('4')
        plt.plot(evoked_pic.times,ERP_all)
        filename_fig = op.join(result_path, 'ERP_all.png')
