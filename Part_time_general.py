def Cat_all(sub):
    ...
    def run_time_general(mod, cat, Category, participant, delta_T, sens,xx,method):
            ...
            times_gen_all[:] = epochs_test.times
    ...
    for xx in [2]:
        for sens in ['meg']:
            ...
            result_all_path='/rds/projects/k/kowalcau-opm-recordings/cross_modal_project/Across_participants/Time_generalization/'+ suffics[xx]+'/'
            ...
            path_to_save=result_path + '/Time_gen/'
            ...
            scores_gen_all = np.zeros((len(['w', 'p']) * len([11, 21, 31]), len(T_full),len(T_full)))
            times_gen_all = np.zeros(len(T_full))
