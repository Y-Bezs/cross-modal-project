...
def Cat_all(sub):
    ...
    def run_category(mod, cat, Category, participant, delta_T, sens, xx, method):
            ...
            epochs_1 = epochs[cat_1].copy().pick_channels(mne.read_vectorview_selection(('Vertex','frontal','temporal','parietal'),info=epochs.info))  #choose epochs to classify
            epochs_2 = epochs[cat_2].copy().pick_channels(mne.read_vectorview_selection(('Vertex','frontal','temporal','parietal'),info=epochs.info))
    ...
    for xx in [2]:
        for sens in ['meg']:#['meg','mag','grad']:
            method = '_' + suffics[xx] + '_' + sens + '_no_occ'

            result_all_path='/rds/projects/k/kowalcau-opm-recordings/cross_modal_project/Across_participants/Category_w_time/'+ suffics[xx]+'/no_occipital/'
            ...
            path_to_save=result_path + '/category_w_time/no_occipital/'
